...
def home():
    ...
    return render_template('homepage.html', users=usersList,saving=savings,todayMonth=todayMonth, prevMonth=prevMonth, todayYear=todayYear, prevYear=prevYear, left=totalLeft,limits=limit,over=over,form=form)
...
def selected():
    ...
    totalLeft = Process.savingsLeft(session['userid'], todayMonth=checkMonth, todayYear=checkYear)
    limit = []
    ...
    return render_template('nov.html', checkMM=months[checkMonth], checkYY=checkYear,
                           users=usersList, count=len(usersList), left=totalLeft,limits=limit,over=over,form=form)
